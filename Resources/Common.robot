...
URL Validation
    ...
    Run Keyword And Continue On Failure  Should Be Equal As Strings  ${curr_url}  ${VURL}    strip_spaces=true
...
Banner Redirection
    [Arguments]     ${locator}    ${locator_id}   ${Sc_Name}
...
Decide Start and End
    [Arguments]  ${st}  ${en}   ${Find-Str}  ${Sheet_name}
    FOR  ${k}    IN RANGE    ${st}   ${en}
        ${Page_Title}=       Read Excel Cell          row_num=${k}      col_num=1  sheet_name=${Sheet_name}
        ${status}  Run Keyword And Return Status  Should Be Equal As Strings  ${page_title}  ${find-str}  strip_spaces=true
        IF  ${status}  
            BREAK  
        END
    END
...
Recharge Popup Verification
    [Arguments]            ${Recharge_locator}      ${Amt_locator}     ${View_Button}   ${RURL}    ${Mobile_No}
    ...
    IF  ${Popup_Presense} == True
        ...
        Wait Until Keyword Succeeds               1 min   0 sec  Wait For And Verify Content     xpath=//p[@class="css-1e3ildx"]  ${Amt}
...
FAQ Section Verification
    ...
    ELSE
        ...
        FOR  ${i}  IN RANGE  1   ${length_of_ques+1}
            ...
            ${presense}=    Run Keyword And Return Status   Element Should Be Visible    xpath=//div[@class='j-accordion-panel__inner' and @aria-hidden='false']//a    
            IF  ${presense} == True
